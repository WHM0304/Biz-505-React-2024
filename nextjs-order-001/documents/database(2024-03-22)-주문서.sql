...
-- WHERE o_pcode1 <> ''     : IS NULL 이면
-- 서브쿼리는 AS를 무조건 설정해줘야한다.
-- 주문-상품 Relation 생성
...
-- 생성한 Relation 데이터를 저장할 Relaion 테이블 생성하기
...
주문원장은 Master Table 이며, 주문-고객 은 Relaion Table 이다.
주문원장에 있는 코드는 주문-고객에 있을 수도 있다 : 0..N
만약 주문-고객에 코드가 있는데 주문원장에 없다 : 있을수 없다.
주문- 고객 을 기준으로 주문번호가 주문원장에 있는지 확인하기
*/

-- 다음 SQL 결과에서 데이터가 한개라도 나오면
-- FK 설정이 불가능하다
SELECT * 
...
ALTER TABLE tbl_order_customer
ADD CONSTRAINT fk_o_c FOREIGN KEY (oc_ccode) 
REFERENCES tbl_customer(c_code);

ALTER TABLE tbl_order_product
ADD CONSTRAINT fk_o_p FOREIGN KEY (op_pcode)
REFERENCES tbl_product(p_code);

/*
주문원장, 주문-상품, 주문-고객 , 상품정보, 고객정보의 형식으로
각 테이블을 분리하고 Relation 을 설정했다. 여기까지 제 3정규화
또는 BCNF(보이스코드정규화)까지 완성되었다.
하지만 실제 사용을 하려고 하면 
다중의 Table이 서로 JOIN 되어야 하는
불편함이 발생하였다.

이런 상황이 되면, 과연 Table 을
 다수로 쪼개는 것이 꼭 좋은 상황인지
살펴보고, Table 간에 통합을 하여 
Table 의 개수를 줄이는 것을 고민해야한다 
이처럼 분리된 테이블을 다시 통합 하는 것을 
제 4정규화, 제 5정규화 과정이라고한다.

tbl_order_product , tbl_order_customer /Relation 을 제거하고
주문원장과 고객정보, 주문원장과 상품정보를 
연결할수 있는 방법을 모색한다.

1. 한개의 주문에는 반드시 고객은 한명 뿐이다.
2. 한개의 주문에는 다수의 상품이 포함될수 있다.
	!!!!!단 , 한개의 주문에 같은 상품이 중복될수 없다.!!!!
*/

-- 기존의 Relation(FK)를 제거
ALTER TABLE tbl_order_customer
DROP CONSTRAINT fk_o_c;

ALTER TABLE tbl_order_customer
DROP CONSTRAINT fk_oc;

ALTER TABLE tbl_order_product
DROP CONSTRAINT fk_o_p;
...
-- 전제 : 이미 주문원장과 주문-고객 테이블에 데이터가 많이있는 상태
...
5. tbl_orderlist.forEach(item) 처럼 실행된다.(쉐도우코드)
...
-- 대하여 실행할수 없도록 초기 세팅이 되어 있다.
-- 현재 tbl_orders 테이블 전체 데이터에 Update 를 실행해야
-- 하므로 잠시 Safe Update 를 해제한다.
...
ALTER TABLE tbl_customer
DROP INDEX c_tel;
DROP INDEX c_tel ON tbl_customer;
