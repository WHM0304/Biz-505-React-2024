...
pure(React) 에서는 모든것이 CSR 이기 때문에 별다른 조치 없이 
내장된 함수, 기능 등을 사용할수 있다.

하지만 NextJS 에서는 컴포넌트가 SSR 에서 동작하느냐, CSR에서 동작하느냐 에 따라
사용할수 있는 함수, 기능이 차이가 있다.
CSR 에서 동작하는 함수를 그냥 사용하면 사용불가 오류가 나타난다.

이 컴포넌트 에서는 useRouter 함수, onClick 기능을 사용하려고 한다.
이러한 경우에는 현재 컴포넌트가 CSR 환경에서 동작하도록 명시해 줘야 한다.
"use client" 를 파일 첫라인에 명시해 주면 이 컴포넌트는 CSR 환경에서 동작한다.
*/
...
export default () => {
  ...
  const onClickHandler = (id) => {
    // alert(id);
    // router.push() 를 사용하면 함수에서 특정한 router 로 전환해준다.
