...
const CustomSearch = ({ customList, setCustomer, setCustomList }) => {
  ...
  const viewList = customList.map((item) => {
    return (
      <li key={item.c_code} c_code={item.c_code} onClick={() => onClickHandler(item)}>
  ...
  return (
    <>
      <ul className={css.cust_list}>{viewList}</ul>
    </>
  );
};
